// POST /api/contact — 헬로100 광고 문의 접수 → Brevo로 담당자 메일 발송
...
const INTERESTS = ['브랜딩(스플래시·팝업)', '프리미엄 배너', '네이티브', '전환 지면', '타겟 푸시', '단가표 요청'];
...
export async function onRequestPost({ request, env }) {
  ...
  const html = `
    <h2 style="font-family:sans-serif">[헬로100 광고문의] ${esc(company)} / ${esc(name)}</h2>
    <table cellpadding="10" cellspacing="0" border="0" style="font-family:sans-serif;font-size:14px;border-collapse:collapse">
      ${rows.map(([k, v]) => `
        <tr>
          <td style="background:#f4f6fa;border:1px solid #ddd;font-weight:bold;white-space:nowrap">${esc(k)}</td>
          <td style="border:1px solid #ddd;white-space:pre-wrap">${esc(v)}</td>
        </tr>`).join('')}
    </table>`;

  const payload = {
    sender: { name: '헬로100 랜딩페이지', email: env.CONTACT_FROM_EMAIL },
    to: [{ email: env.CONTACT_TO_EMAIL }],
    subject: `[헬로100 광고문의] ${company} / ${name}`,
    htmlContent: html,
  };
